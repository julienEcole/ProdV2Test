...
class HFSQL{
	private $_nomBdd; //l'attribut contenant le nom de ma base de donnée
    ...
    public function __construct($Serveur,$Bdd = "test",$Utilisateur = "root",$mdp = ""){ //mon constructeur vérifié
		...
		if(is_string($mdp)) {
			...
			if($this->_nomServeur != "" &&  $this->_nomBdd != "") {		//pas la peine de tenter de se connecter si on a pas mis de nom de bdd ni de serveur
				...
				catch(PDOexception $e){	//en cas d'erreur on est sensé avoir un message
					echo ("Erreur e : ".$e->getMessage()."<br />La connexion a la BDD a échoué, vérifiez vos arguments <br />");
				}
			}
			else{
				throw new PDOException("PDO null veuillez vérifier les parametres précédents");
			}
		}
		else {
			$_mdp = "";
			echo "tous les champs n'ont pas été rempli, connexion actuellement impossible avec la bdd ";
		}
	}
